...
- `src/data/` — manifest generation ([create_manifest.py](src/data/create_manifest.py)), PyTorch `Dataset` ([dataset.py](src/data/dataset.py)), and optional synthetic defect generation ([generate_synthetic_defects.py](src/data/generate_synthetic_defects.py))
...
## Synthetic Defect Generation (optional, exploratory)

[src/data/generate_synthetic_defects.py](src/data/generate_synthetic_defects.py) generates synthetic defective images from a category's `train/good` images via ControlNet (Canny-edge-conditioned) + Stable Diffusion inpainting: it Canny-edges a pristine image to preserve its physical geometry, masks a small randomly-placed region (`scratch`/`blob`/`patch`), and inpaints a text-prompted defect only inside that mask — producing a synthetic defective image with an exact ground-truth mask, generated entirely from `train/`, never derived from `test/`.

**Motivation:** both the baseline good-vs-defective classifier and the per-category defect-type classifier have to be trained on splits carved out of the labeled `test/` set, because MVTec's `train/` only contains `good` images (see "misleadingly perfect classifier" and defect-type-classifier findings elsewhere in this README). Genuinely novel, train-only synthetic defects are a way to break that dependency instead of just working around it with better splitting/regularization.

```bash
pip install -r requirement-synthetic.txt   # diffusers, transformers, accelerate (optional, not in requirement.txt)
python -m src.data.generate_synthetic_defects --category screw \
  --defect-type synthetic_rust --prompt "deep rust, corrosion damage" --num-images 10
```

This writes synthetic images/masks to `data/synthetic_defects/<category>/<defect_type>/` and a manifest-schema-compatible `data/manifests/<category>_synthetic.csv` — kept as a separate file from the real `data/manifests/<category>.csv` so synthetic images are never silently mixed into the canonical, tracked manifest.

**Status — implemented and unit-tested, not yet run end-to-end with the real model.** `diffusers`/`transformers`/`accelerate` and the multi-GB ControlNet + Stable-Diffusion-inpainting weights are deliberately kept out of the main `requirement.txt` (a heavy, single-purpose dependency most of the pipeline doesn't need), and actually running the diffusion model is slow without a CUDA GPU (this Mac's Apple Silicon MPS backend, or CPU, both work with `diffusers` but noticeably slower — the module already resolves `cuda > mps > cpu` and uses fp32 off-CUDA since fp16 isn't reliably supported on MPS/CPU for every op these pipelines use). Everything *not* requiring the diffusion weights — mask generation (`make_random_mask`), the Canny control image, file saving, and manifest writing — is covered by [tests/test_synthetic_defects.py](tests/test_synthetic_defects.py), including a full `generate_synthetic_defects()` run with a stub pipeline injected via `pipeline_factory` (echoes the mask back as the "generated" image) so the surrounding plumbing is verified without downloading any model weights. Actually generating synthetic images and validating whether they measurably help the baseline/defect-type classifiers is future work.
