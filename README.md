...
5. Turns the score into a **Normal / Defective** verdict by comparing against the stored image-level Youden threshold.
...
| Transistor | 0.93 / 0.91 | **0.998** | 0.961 | 0.321 | 0.463 |
...
- **A plain min-max colored heatmap doesn't visually match the actual localization boundary.** Even with background masked out, ordinary (sub-threshold) screw texture — like normal thread ridges — still has a non-zero, spatially-varying anomaly score, so a full-range jet colormap can render it yellow/green and look like "a big defect" even when it is below the calibrated pixel threshold. Anchoring the colormap to that pixel threshold (`normalize_map_threshold` in [src/visualization/heatmap.py](src/visualization/heatmap.py)) — compressing lower values into the cool half and only letting predicted anomaly pixels read as hot — makes the heatmap agree with severity and defect-crop localization. The separate image threshold controls only the Normal/Defective verdict.
...
- **On a 40-image dataset, a single misclassification swings accuracy by 8–12.5 points, so the earlier transistor numbers above were one noisy sample, not a reliable estimate.** `train_defect_classifier.py` gained `--holdout-per-class`/`--cross-validation --folds` so a tiny per-category dataset can be evaluated properly: 2 images/defect type (8 total) are reserved as an untouched final test set first, stratified 4-fold CV runs only on the remaining 32 (24 train/8 val per fold — exact, no rounding, since 8/class ÷ 4 folds = 2/class/fold) to pick settings, then one final model is fit on all 32 dev images and evaluated exactly once on the 8-image holdout. Every fold/holdout metric is reported as both a percentage and a raw `correct/total` count (e.g. "6/8 (75.0%)") specifically because at n=8 a single error is 12.5 points. Re-running the earlier frozen-vs-fine-tuned comparison with this design gave a much clearer, lower-variance answer than the single 12-image val split had: frozen backbone CV accuracy **31.25% ± 16.1%** std (10/32 out-of-fold) vs. full fine-tune **65.6% ± 12.0%** std (21/32 out-of-fold) — fine-tuning is not just better on average but also more stable across folds (relative std 0.51 vs. 0.18). The final holdout fit (10 epochs, matching the CV folds' typical best_epoch of 9–10) scored **6/8 (75.0%)**, comfortably inside the CV's estimated range — good evidence the 4-fold CV mean was a reasonably unbiased predictor of held-out generalization despite each fold only having 8 validation images. Both of its misses (`bent_lead`→`cut_lead` and `damaged_case`→`cut_lead`) were absorbed by the same class, matching `cut_lead`'s role as a "confusion sink" during CV (precision 0.5, recall 1.0 on the holdout — it never missed a true `cut_lead`, but also caught every other class's mistakes).
